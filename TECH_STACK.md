...
- 每日新词/复习词额度分开设置，复习优先于新词；复习池内部按到期日推送（逾期最久的先出，同一天到期的再按熟悉度从低到高）
- 两个额度互不干扰：新词只受新词额度约束，不因为复习有积压而停发。可持续的前提是**复习额度 >= 4 × 新词额度**（一个词升到满级要 4 次复习，N 新词/天在稳态下产生约 4N 次/天的复习需求），这条由用户在设置里自己保证；比例失衡时积压会增长，条数经 `/api/next` 的 `review_backlog` 返回并显示在完成画面上
